...
import { createWorld, DIFFICULTIES, FACTIONS, getLevelConfig, MAX_LEVEL, marchingSoldiers, readProgress, recordWin, progressKey, sendTroops, territoryAt, tickWorld } from '../lib/conquest'
...
function initialize() {
  let storage
  try { storage = window.localStorage } catch { /* Continue without saved records. */ }
  const records = { child: readProgress(storage), adult: readProgress(storage, 'adult') }
  return { mode: 'audience', difficulty: 'child', records, selected: records.child.selected, level: 1, world: null, round: 0 }
}

function reducer(state, action) {
  const progress = state.records[state.difficulty]
  if (action.type === 'audience') return { ...state, mode: 'audience', world: null }
  if (action.type === 'difficulty' && Object.hasOwn(DIFFICULTIES, action.difficulty)) {
    const profile = state.records[action.difficulty]
    return { ...state, difficulty: action.difficulty, mode: 'menu', selected: profile.selected, level: Math.min(MAX_LEVEL, profile.cleared[profile.selected] + 1), world: null }
  }
  if (action.type === 'select') return { ...state, selected: action.team, level: Math.min(MAX_LEVEL, progress.cleared[action.team] + 1), records: { ...state.records, [state.difficulty]: { ...progress, selected: action.team } } }
  if (action.type === 'level' && action.level <= progress.cleared[state.selected] + 1) return { ...state, level: action.level }
  if (action.type === 'start' && state.mode !== 'audience') return { ...state, mode: 'battle', world: createWorld(state.level, state.selected, action.portrait, state.difficulty), round: state.round + 1 }
  if (action.type === 'menu') return { ...state, mode: 'menu', world: null, level: Math.min(MAX_LEVEL, progress.cleared[state.selected] + 1) }
  if (action.type === 'next' && state.world?.status === 'won' && state.level < MAX_LEVEL) return { ...state, level: state.level + 1, world: createWorld(state.level + 1, state.selected, action.portrait, state.difficulty), round: state.round + 1 }
  ...
  if (action.type === 'tick' && state.world?.status === 'playing') {
    ...
    return { ...state, world, records: world.status === 'won' ? { ...state.records, [state.difficulty]: recordWin(progress, world) } : state.records }
  }
  return state
}
...
function MapPreview({ player, difficulty }) {
  const preview = useMemo(() => createWorld(1, player, false, difficulty), [player, difficulty])
...
export default function Conquest() {
  ...
  const { records, difficulty, selected, level, world, mode } = state
  const progress = records[difficulty], audience = DIFFICULTIES[difficulty]
  const audio = useConquestAudio(world, help, state.round)
  const team = FACTIONS[selected], config = getLevelConfig(level, difficulty)
  ...
  useEffect(() => {
    for (const [difficulty, progress] of Object.entries(records)) {
      try { localStorage.setItem(progressKey(difficulty), JSON.stringify(progress)) } catch { /* Storage is optional; the current session still progresses. */ }
    }
  }, [records])
  useEffect(() => {
  ...
  return <main className={`ct-container ${mode === 'battle' ? 'ct-playing' : ''}`} style={{ '--team-color': team.color, '--team-light': team.light }}>
    <div className="ct-shell">
      ...
      {mode === 'audience' ? <section className="ct-audience" aria-labelledby="ct-audience-title">
        <span className="ct-eyebrow">여섯 영웅의 한판 승부</span>
        <h1>내가 <span>다 먹었다!</span></h1>
        <h2 id="ct-audience-title">오늘은 누가 대장이 될까요?</h2>
        <p>나에게 맞는 모험을 골라 주세요.</p>
        <div className="ct-audience-grid">
          {Object.entries(DIFFICULTIES).map(([key, option]) => <button key={key} className={`ct-audience-card ct-audience-${key}`} aria-label={`${option.label} 모드로 시작`} onClick={() => { setHelp(false); audio.play('select', key === 'child' ? 0 : 3); dispatch({ type: 'difficulty', difficulty: key }); window.scrollTo({ top: 0 }) }}>
            <span className="ct-audience-symbol" aria-hidden="true">{option.symbol}</span>
            <strong>{option.label}</strong><b>{option.subtitle}</b><p>{option.description}</p>
            <span className="ct-audience-go">{option.label} 모드로 시작 <span aria-hidden="true">→</span></span>
          </button>)}
        </div>
        <p className="ct-audience-note">각 모드에서 1~20탄에 도전해요.<br />아이와 어른의 모험 기록은 따로 보관돼요.</p>
      </section> : mode === 'menu' ? <>
        <div className="ct-mode-strip"><span className={`ct-mode-badge ct-mode-${difficulty}`}>{audience.symbol} {audience.label} 모드</span><span>{audience.subtitle}</span><button onClick={() => { setHelp(false); dispatch({ type: 'audience' }); window.scrollTo({ top: 0 }) }} aria-label="아이 · 어른 다시 선택">모드 바꾸기 ↔</button></div>
        <section className="ct-intro"><div className="ct-intro-copy"><span className="ct-eyebrow">여섯 영웅의 한판 승부</span><h1>내가 <span>다 먹었다!</span><i aria-hidden="true">✦</i></h1><p>내가 좋아하는 팀과 함께,<br />작은 섬 하나부터 온 세상을 차지해 봐!</p><div className="ct-feature-pills"><span>☝ 쭉 끌어서 출정</span><span>⚑ 6개 진영</span><span>✦ 20탄의 모험</span></div></div><MapPreview player={selected} difficulty={difficulty} /></section>
        ...
        <section className="ct-mission-section" aria-label="모험 선택"><div className="ct-section-heading"><h2><span>02</span> 오늘의 모험</h2><span className="ct-save-note">진행은 모드·팀별로 자동 저장돼요</span></div><div className="ct-mission-row"><span className="ct-level-stamp">{String(level).padStart(2, '0')}<small>/ 20</small></span><div className="ct-mission-copy"><span>{config.region}</span><h3>{config.name}</h3><p>땅 {config.count}개 · 6팀 대결{difficulty === 'adult' ? ' · 치열한 전략 승부' : level <= 3 ? ' · 천천히 연습해요' : ''}</p></div><details className="ct-level-picker"><summary>다른 탄 고르기 <span>⌄</span></summary><div className="ct-level-popover"><p>앞 탄을 정복하면 다음 모험이 열려요.</p><div className="ct-level-grid">{Array.from({ length: MAX_LEVEL }, (_, index) => { const number = index + 1, locked = number > progress.cleared[selected] + 1; return <button key={number} disabled={locked} aria-label={`${number}탄${locked ? ', 잠김' : ''}`} aria-pressed={level === number} onClick={(event) => { dispatch({ type: 'level', level: number }); event.currentTarget.closest('details').open = false }}><span>{number}</span><small>{locked ? '잠김' : progress.stars[`${selected}-${number}`] ? '★'.repeat(progress.stars[`${selected}-${number}`]) : '도전'}</small></button> })}</div></div></details></div></section>
        ...
        <div className="ct-battle-heading"><div><span className="ct-eyebrow"><span className={`ct-mode-badge ct-mode-${difficulty}`}>{audience.label} 모드</span> {config.region} · {level} / 20탄</span><h1>{config.name}</h1></div><div className="ct-battle-clock" aria-label="진행 시간">◷ {formatTime(world.time)}</div></div>
